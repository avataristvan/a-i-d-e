...
from aide.features.symbol_history.domain.ports import GitPort, CommitInfo
...
class SymbolHistoryUseCase:
    ...
    def execute(self, file_path: str, symbol_name: str, limit: int = 10) -> list[CommitInfo]:
        content = self.file_system.read_file(file_path)
        start, end = self.locator.locate(file_path, content, symbol_name)
        return self.git.log(file_path, start, end, limit)
