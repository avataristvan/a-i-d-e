...
9. **Sub-Agent Delegation**: Even as a Standalone agent, you should delegate complex boilerplate or isolated logic generation to AIDE's internal sub-agents to save time and tokens. Use `scaffold-feature`, `project-dto`, `register-dependency`, and `implement-logic` to perform secure, deterministic generation.
10. **Human Interface**: If a human asks to "show the code" or "show the implementation", use `aide.py read` to provide a line-numbered output to facilitate precise communication.
